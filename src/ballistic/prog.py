...
class Spacer:
    ...
    def combine(self, spaces : list[SubSpace[T]]) -> SearchSpace[T]:
        ...
        for i in range(len(self.input_data)):
            result = RealVal(0) 
            for (control, space) in control_spaces:
                result = If(control, space.outcol[i], result)
            outcol.append(result)
    ...
    def to_body(self, plates_in_scope : dict[str, list[list[ArithRef]]], vars_in_scope : dict[str, list[ArithRef]]) -> SearchSpace[BodyOption]:
        ...
        if self.body_fuel > 0: 
            ...
            def scope_sample():
                src, _= self.to_dist(plates_in_scope, vars_in_scope)   
                id = self.fresh_var() 
        ...
        def scope_final():
            ...
            content, _ = self.to_dist(plates_in_scope, vars_in_scope)   
    ...
    def to_expr(self, plates_in_scope : dict[str, list[list[ArithRef]]], vars_in_scope : dict[str, list[ArithRef]]) -> SubSpace[ExprTree]:
        ...
        for i in range(len(self.input_data)):
            for ext in exts:
                outcol[i] = outcol[i] + ext.outcol[i]
    ...
    def to_prod(self, plates_in_scope : dict[str, list[list[ArithRef]]], vars_in_scope : dict[str, list[ArithRef]]) -> SubSpace[ProdTree]:
        ...
        for i in range(len(self.input_data)):
            for factor in factors:
                outcol[i] = outcol[i] * factor.outcol[i]
    ...
    def to_ext(self, plates_in_scope : dict[str, list[list[ArithRef]]], vars_in_scope : dict[str, list[ArithRef]]) -> SearchSpace[ExtOption]:
        ...
        def scope_minus():
            ...
            spaces.append(SubSpace(
                ...
                dlen = arg.dlen ,
                outcol = [-1 * output for output in arg.outcol],
                constraint=arg.constraint
            ))
        scope_minus()
    ...
    def to_atom(self, plates_in_scope : dict[str, list[list[ArithRef]]], vars_in_scope : dict[str, list[ArithRef]]) -> SearchSpace[AtomOption]:
        ...
        def scope_params():
            for id in (self.params):
                ...
                spaces.append(SubSpace(
                    ...
                    dlen = RealVal(1),
                    outcol = [RealVal(row[pid].item()) for row in self.input_data],
                    constraint=And()
                ))
        scope_params()
        ...
        def scope_float():
            ...
            spaces.append(SubSpace(
                ...
                dlen = RealVal(2),
                outcol = [weight for _ in self.input_data],
                constraint=And()
            ))
...
def synthesize_body(search_space : SearchSpace[BodyOption], output_data) -> tuple[str, float]:
    ...
    noise_total = Sum([
        ...
        for output, prediction in zip(output_data, search_space.outcol)
    ])
