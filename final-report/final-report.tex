...
To measure the correctness of a found program $f$, we define a cost in terms of its 
description length $-\text{log}\ \text{L}(f)$ and noise $-\text{log}\ \text{P}(y|f(x)) $.
A length is associated with each production rule in the grammar.
Summing together the length across all nodes in the found program results 
in the total description length . 
