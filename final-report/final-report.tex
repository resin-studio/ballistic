...
The system encodes this cost definition in Z3.
\[
    cost = - \text{log}\ \text{L}(f) + \sum_{i=1}^N - \text{log}\ \text{P}(y_i|f(x_i))
\]
...
\begin{array}{l}
  \text{search}(\phi, \sigma) \triangleq \\
    ...
    \cdot\ \sigma' \Rightarrow \text{search}(\phi \wedge (cost < \sigma'[cost]), \sigma')
      % \hspace{6mm}\begin{array}{@{} l}
